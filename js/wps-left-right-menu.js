  ...
  var privateMethods = {
    ...
    addPrefix: function($element) {
      ...
      if(typeof elementClass === 'string' && '' !== elementClass && 'wps-inner' !== elementClass) {
        $element.attr('class', elementClass.replace(/([A-Za-z0-9_.\-]+)/g, 'wps-class-$1'));
      }
...
"use strict";
	$(document).ready(function(){
		// DROPDOWN MENU
		$("#wps-right ul > li > a").on("click", function(e){
			if($(this).parent().has("ul")) {
			  e.preventDefault();
			}
			if(!$(this).hasClass("open")) {
			  // hide any open menus and remove all other classes
			  $("#wps-right ul li ul").slideUp(350);
			  $("#wps-right ul li a").removeClass("open");
